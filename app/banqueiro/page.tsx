...
import { MapPin, CheckCircle2, Clock, CreditCard, Package } from 'lucide-react';
...
export default function BanqueiroDashboard() {
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      const { data: profile } = await supabase.from('profiles').select('id, local_id').eq('id', user.id).single();
      if (!profile) return;

      const { data: accounts } = await supabase.from('accounts').select('status, tpa_status').eq('banqueiro_id', profile.id);
      if (accounts) {
        setStats({
          totalPacotes: accounts.length,
          tpaEntregues: accounts.filter(a => a.tpa_status === 'entregue').length,
          tpaPendentes: accounts.filter(a => a.tpa_status === 'pendente').length,
        });
      }

      const hoje = new Date().toISOString().split('T')[0];
      const { data: presenca } = await supabase.from('presences').select('*').eq('profile_id', profile.id).eq('data', hoje).single();
  ...
  const marcarPresenca = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      const { error } = await supabase.from('presences').insert({
        profile_id: profile.id, data: hoje, entrada: hora, latitude, longitude,
        mercado_id: profile.local_id, status: 'no_local', pontualidade: 'no_horario', origem: 'gps'
      });

      if (!error) { alert("Presença marcada com sucesso!"); window.location.reload(); }
  ...
  if (loading) return <div className="flex h-64 items-center justify-center">A carregar dashboard...</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {presencaHoje ? (
          ...
          <Button onClick={marcarPresenca} className="bg-bci-magenta hover:bg-bci-magenta/90 text-white">
            <MapPin className="mr-2" size={18} /> Marcar Presença (GPS)
          </Button>
        )}
      </div>
      ...
      {/* Localização em tempo real */}
      <Card>
        <CardHeader><CardTitle className="text-lg font-semibold text-gray-800">A minha localização agora</CardTitle></CardHeader>
        <CardContent>
          {mapsEmbedSrc ? (
            <iframe
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-6 flex items-center space-x-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900">{stats.totalPacotes}</h3>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6 flex items-center space-x-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-500">TPA's Entregues</p>
        ...
        <Card>
          <CardContent className="p-6 flex items-center space-x-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-500">TPA's Pendentes</p>
              <h3 className="text-2xl font-bold text-gray-900">{stats.tpaPendentes}</h3>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader><CardTitle className="text-lg font-semibold text-gray-800">TPA's pendentes e entregues</CardTitle></CardHeader>
        <CardContent className="h-64">
          {stats.totalPacotes > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={tpaData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                  {tpaData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                </Pie>
                <RechartsTooltip /><Legend verticalAlign="bottom" height={36} />
              </PieChart>
            </ResponsiveContainer>
          ) : <div className="flex h-full items-center justify-center text-gray-400">Sem dados suficientes</div>}
        </CardContent>
      </Card>
